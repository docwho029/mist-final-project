async function loadItems() {
  ...
  for(const categoryName of categoryList) {
    ...
    category.className = "section__product-row-1";

    const res2 = await fetch(`https://dummyjson.com/products/category/${categoryName}`);
    const productsList = await res2.json();
    ...
    category.innerHTML = `
        <h2 class='heading-product-slider' id='category-name'>${ categoryName }</h2>
        <section class="slidey">
          <div class="product-swiper swiper">
            <div id="buddyYoureCooked" class="swiper-wrapper swiper-wrapper-two">
              ${productSlides}
            </div>
      
              <div class="swiper-button-next product-swiper-button-next !right-0 !mt-0">
              ...
              <div class="swiper-button-prev product-swiper-button-prev !left-0 !mt-0">
...
document.addEventListener("click", e => {
  if(e.target.matches(".fa-heart")) {
    ...
    // if( document.querySelector(".product-popup").classList.contains("hidden") && document.querySelector(".popup").classList.contains("hidden")) {
    //   // no menus open, product card in home page is clicked
    
    // } else if(document.querySelector(".product-popup").classList.contains("hidden") ) {
      //   // product popup open
      
      // }

    //const productClicked = e.target.closest(".swiper-slide-two");
    const productClicked = e.target.closest("[data-product-id]");
